...
public class UserInfoController{
	...
	/*
	 * 비즈니스 로직(중요한 로직을 수행하기위해 사용되는 서비르를 메모리에 적재(싱글톤패턴 적용됨)
	 */
	...
	public String getUserLoginCheck(HttpSession session, HttpServletRequest request, HttpServletResponse response, ModelMap model) throws Exception{
		...
		//로그인 처리 결과를 저장하는 변수(로그인 성공: 1, 아이디, 비미번호 불일치로인한 실패 : 0, 시스템에러 :2)
		int res = 0;
		...
		try {
			...
			/*
			 * #############################################################
			 * 반드시, 값을 받았으면, 꼭 로그를 찍어서 값이 재대로 들어오는지를 파악해야함
			 * 						반드시 작성할것
			 * #############################################################
			 * 
			 */
			...
			/*
			 * #############################################################
			 * 			웹(회원정보 입력화면)에서 받는 정보를 DTO 변수에 저장시작!!
			 * 
			 ...
			 * 
			 * 		무조건 웹으로 받은 정보는 DTO에 저장해야 한다고 이애하길 권함
			 * #############################################################
			 */
			 ...
			 * 
			 * 세션은 톰켓(was)의 메모리에 존재하며, 웹 사이트에 접속한 사람(연결된 객체) 마다 메모리에 값을 올린다.
			 * 
			 * 예) 톰켓에 100명의 사용자가 로그인했다면, 사용자 각각 회원아이디를 메모리에 저장하며,
			 * 메모리에 저장된 객체의 수는 100개이다.
			 * 따라서 과도한 세션은 톰켓의 메모리 부하를 발생시켜 서버가 다운되는 현상이 있을수 있기 때문에,
			 * 최소한으로 사용하는 것을 권장한다.
			 * 
			 * 스프링에서 세션을 사용하기 위해서는 함수명의 파라미터HttpSession session 존재해야한다.
			...
			if (res==1) { //로그인 성공
				...
				/*
				 * 세션에 회원아이디 저장하기, 추후 로그인 여부를 체크하기위해 세션에 값이 존재하는지 체크한다.
			...
			/*
			 * 로그인 처리결과를 jsp에 전달하기 위헤 변수 사용
			 * 숫자유형의 데이터 타입은 값을 전달하고 받는데 불폄함이 있어
			 * 문자 유형(String)으로 강제 형변환을하여 jsp에 전달한다.
			 */
	...
	public String insertUserInfo(HttpServletRequest request, HttpServletResponse response,
		...
		try {
			 ...
			 * 
			 * 무조건 웹으로 받는 정보는 DTO에 저장하기 위해 임시로 String변수에 저장함
			 */
			/*
			 * ##################################
			...
			/*
			 * ###############################
			 * 웹(회원정보 입력화면)에서 받는 정보를 DTO에 저장하기끝!!
			 * 
